-- [[
--  Chloe X | Community
--  (c) 2025 Neuro | Chloe X Development
...
-- Neuro.Config.CreateConfig.Folder
if not isfolder("Neuro X") then makefolder("Neuro X") end
if not isfolder("Neuro X/Config") then makefolder("Neuro X/Config") end
...
-- Neuro.Config.Load.LoadConfigFromFile
...
-- Neuro.Config.Load.LoadConfigElements
...
function ChloeX:MakeNotify(Notify)
    Notify             = Notify or {}
    Notify.Title       = Notify.Title or "Neuro X"
    ...
    -- Neuro.MakeNotify.OpenAnimation
    TweenService:Create(
        CHX[4],
        TweenInfo.new(Notify.Time, Enum.EasingStyle.Back, Enum.EasingDirection.InOut),
...
function ChloeX:Window(Flag)
    ...
    GuiConfig.Title        = GuiConfig.Title or "Neuro X"
    GuiConfig.Footer       = GuiConfig.Footer or "Neuroe :3"
    ...
    CHX[24].Activated:Connect(function()
        ...
        CHX[49].MouseButton1Click:Connect(function()
            CHX[15]:Destroy()
            if CoreGui:FindFirstChild("NR-Button") then
                CoreGui["NR-Button"]:Destroy()
            end
        end)
